...
const OTPModal: React.FC<OTPModalProps> = ({
  ...
  return (
    ...
    >
      {hasCloseButton && (
        <CloseButton onClick={onClose}>{crossSVG}</CloseButton>
      )}
      <ContentContainer gap={gap}>
        <TitleName
          svgContent={verifyOTPSVG}
          title={title}
          height={titleHeight}
          width={titleWidth}
          gap={titleGap}
        ...
        <OTPButtonandTimerDiv>
          ...
          <MultiFunctionButtonComponent
            ...
            width="228px"
          />
          <Timer initialTime={timeLeft} svgIcon={ClockIconSVG} />
          <ResendDiv>
            Didn’t received?{" "}
            <Resend timeLeft={timeLeft} onClick={() => setTimeLeft(60)}>
              Resend
            </Resend>
          </ResendDiv>
        </OTPButtonandTimerDiv>
